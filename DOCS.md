...
**Соседние гейтвеи и прямые чартеры (предлагать, не решать за юзера).** Самый дешёвый/быстрый вход не всегда через ближайший к цели аэропорт. У соседнего города бывает прямой рейс, которого нет у «очевидного» (пример: прямого MOW→JRO нет, а прямой чартер MOW→ZNZ есть + дешёвый хоп ZNZ→Аруша — экономия ~65к ₽ и пересадка). Из РФ на пляжные хабы часто летают прямые чартеры (Azur / Nordwind / Red Wings). Перед фиксацией маршрута назвать 1–2 альтернативных гейтвея, спросить юзера и по согласию прогнать доп-поиски + глянуть strip «цены на соседние даты».
...
**Какой Ostrovok — corp или публичный (спросить один раз).** `corp.ostrovok.ru` (портал «Командировки») требует агентского логина, но даёт нетто-цены в ₽, оплату рос-картой и иногда инвентарь, которого нет на публичном (агентские цены часто дешевле даже официального сайта отеля). Публичный `ostrovok.ru` — гостевые цены без логина, для всех. В начале работы с отелями спросить, какой использовать (скилл шарится — не у всех есть corp), default — публичный. **Не угадывать slug региона:** один отель может лежать под неожиданным (Riu Palace Swahili → регион `kendwa`, не `nungwi`/`zanzibar`) — вбивать название в поиск/автокомплит и брать подсказку (получишь правильный регион + `mid`).
...
<body>
  <div class="container">
    ...
    <div id="t-budget"><!-- Budget (смета) — опц., только если задан budget --></div>
...
**Смета (`budget`, опц.).** Помимо summary есть опциональный `budget`-блок: построчная смета (`items[]` + один выделенный `total`) для категорий, которые не влезают в flights/hotels totals — тур/сафари-оператор, трансферы, виза, страховка, чаевые. Рендерится карточкой после summary, попадает в XLSX, из price-free PDF исключён. В `total.value` можно дать диапазон, когда цифры ещё оценочные.
